...
class ProjectsTest < ActionController::IntegrationTest
  ...
  context "Unlogged in user" do
    ...
    should "be able to get 404 fer unknown project page" do
      get("projects/someunknownproject")
      assert_response_404
    end
  ...
  context "User contributing to project" do
    ...
    should "be able to change unvalidated contributors email" do
      ...
      assert_project_contains "someoneelse@mail.com", true
    end

    should "be able to add  email to unvalidated contributor" do
      add_contributor "Some Random Name"
      assert_select 'li.not-validated', /.*Some Random Name.*/ do |elem|
        assert_select 'a.edit-unvalidated'
      end
      uvc = UnvalidatedContributor.find_by_name("Some Random Name")
      get edit_project_unvalidated_contributor_path(@project, uvc)
      assert_response_ok
      fill_in 'email', :with=> "someoneelse@mail.com"
      fill_in 'name', :with=> "Shnoggle"
      click_button
